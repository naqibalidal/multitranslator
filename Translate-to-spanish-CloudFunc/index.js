...
functions.cloudEvent('helloPubSub', async cloudEvent => {
  ...
  console.log('Function started translating into spanish');
  const target= 'es';
  ...
  const translatedFileName = `${generatedId}-spanish.txt`;

  // Define Cloud Storage folder paths
  const translatedFilePath = `spanish/${translatedFileName}`;
  ...
  console.log(`Translation file locations saved to Firestore with ID: ${generatedId}`);
});